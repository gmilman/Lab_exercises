...
select	review_score, review_category, count(review_id) from order_reviews
group by review_score, review_category /*sql doesnt know that the score and category are linked,
thus need to inlcude both columns in the group by, anything that is not aggreagted (e.g. count ect)
needs to be in the group by function*/
order by 1 desc
limit 1;
